    ...
    for itr, data in enumerate(data_loader):
            ...
            with torch.no_grad():
                ...
                if not training:
                    if attend_across_segments:
                        ...
                        outputs = [
                            [classifier(ost) for ost in os] for os in outputs
                        ]
            if training:
                if attend_across_segments:
                    outputs = [classifier(o) for o in outputs]
        ...
        if attend_across_segments:
            ...
            loss = (
                sum(
                    [
                        sum([criterion(ost, labels) for ost in os])
                        for os in outputs
                    ]
                )
                / len(outputs)
        ...
        with torch.no_grad():
            if attend_across_segments:
                outputs = sum([F.softmax(o, dim=1) for o in outputs]) / len(
                    outputs
                )
            else:
                outputs = (
            ...
            top1_acc = (
                100.0
                * outputs.max(dim=1).indices.eq(labels).sum()  # type: ignore
                / batch_size
            )
...
def init_opt(
    ...
    scaler = torch.cuda.amp.GradScaler() if use_bfloat16 else None  # type: ignore
